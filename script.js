...
const consumptionData = {
    day: [1.2, 1.0, 0.8, 0.7, 0.9, 1.1, 1.5, 2.1, 2.5, 2.8, 2.6, 2.3, 2.0, 1.8, 1.6, 1.4, 1.6, 2.0, 2.4, 2.7, 2.5, 2.2, 1.8, 1.4],
    week: [18.5, 17.2, 19.1, 20.3, 22.5, 16.8, 15.2],
    month: [450, 420, 480, 460, 490, 510, 530, 520, 500, 480, 460, 440, 420, 400, 410, 430, 450, 470, 490, 510, 530, 520, 500, 480, 460, 440, 420, 430, 450, 470]
};
    ...
    window.deviceChart = new Chart(deviceCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: [
                    '#2E86AB', '#A23B72', '#4CAF50', '#FF9800', '#F44336', '#6C757D'
                ],
                borderWidth: 1
            }]
        },
...
function getDeviceIcon(type) {
    switch(type) {
        case 'appliance': return 'fa-refrigerator';
        case 'electronics': return 'fa-tv';
        case 'lighting': return 'fa-lightbulb';
        case 'heating': return 'fa-temperature-low';
...
function updateDeviceChart() {
    ...
    const updatedData = sampleDevices
        .filter(d => d.status)
        .map(d => ({
            ...d,
            usage: d.usage * (0.9 + Math.random() * 0.2) // Randomize by ±10%
        }));
    
    window.deviceChart.data.labels = updatedData.map(d => d.name);
    window.deviceChart.data.datasets[0].data = updatedData.map(d => d.usage);
    window.deviceChart.update();
}

// Update stats with random variations to simulate real-time data
function updateStats() {
    // Current usage
    const currentUsage = document.getElementById('current-usage');
    const currentValue = parseFloat(currentUsage.textContent);
    ...
    const todayValue = parseFloat(todayUsage.textContent);
